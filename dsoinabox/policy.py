...
def evaluate(run: ScanRun, options: ScanOptions) -> PolicyResult:
    result = PolicyResult(
        ...
        fail_on_secrets_mode=options.fail_on_secrets_mode,
    )
    ...
    result.secrets_verified = verified
    gated_secrets = verified if options.fail_on_secrets_mode == "verified" else secrets
    result.secrets_failed = bool(options.fail_on_secrets and gated_secrets)
